...
struct CityDetailView: View {
    ...
    var body: some View {
        NavigationView{
            ScrollView {
                VStack(spacing: 24) {
                    VStack(alignment: .leading, spacing: 16) {
                        Text("名前")
                            .font(.headline)
                        TextField("名前を入力", text: $viewModel.updatedCurrentCity.name)
                            .padding()
            ...
            .navigationTitle(viewModel.updatedCurrentCity.name)
            .navigationBarTitleDisplayMode(.inline)
            .toolbar {
                ToolbarItem(placement: .navigationBarTrailing) {
                    Button("更新") {
                        viewModel.updateExistingCity(city: viewModel.updatedCurrentCity)
                        dismiss()
                    }
                    .bold()
